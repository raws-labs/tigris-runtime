...
static uint8_t *run_chain_plan_with_flag(
    ...
    uint32_t *out_len, unsigned long *out_rows)
{
    *out_len = 0;
    *out_rows = 0;
...
#endif
    uint8_t *out = run_plan_capture_output(&plan, out_len);
#ifdef TIGRIS_COUNT_KERNEL_ROWS
...
static void test_exec_chain_tiled_line_buffered_rolls(void)
{
    ...
    uint8_t *out_on = run_chain_plan_with_flag(
        plan_bytes, plan_len, 1, &len_on, &rows_on);
#ifdef TIGRIS_COUNT_KERNEL_ROWS
    ...
    printf("    tile_h=%d out_h=%d tiles=%d last_tile_h=%d\n",
           eff_tile_h, out_h, eff_tiles, last_tile_h);
    ...
    uint8_t *out_off = run_chain_plan_with_flag(
        plan_bytes, plan_len, 0, &len_off, &rows_off);
